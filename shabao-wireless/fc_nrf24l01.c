...
static void NRFThread(void *arg) {
...
void NRFInit(void)
{
     ...
     */
    chBSemObjectInit(&NRFSemIRQ, TRUE); /* Locks the thread until an IRQ arrives                                                        */
    chBSemObjectInit(&NRFSemRX, TRUE); /* Semaphore initialized as taken, because no data is ready yet            */
    chBSemObjectInit(&NRFSemTX, FALSE); /* Semaphore initialized as free, because transmit channel is open    */
